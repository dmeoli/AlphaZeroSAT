...
def parse_args():
    ...
    parser.add_argument("--num-steps", type=int, default=int(1e5), help="total number of steps to run the environment for")
    ...
    parser.add_argument("--model_name_freq", type=int, default=1e4, help="save model with a different name every time this many iterations are completed")
    parser.add_argument("--model-dir", type=str, default=None, help="load model at this directory")
...
class data_shuffler_v2:
	...
	def __init__(self, input_dumpName, sortM = False, nbatch = -1):
		...
		temp = list(map(lambda x: x.toarray()[:,:,None], data_dict["states"])) # now the states are list of full numpy matrix (3-D)
		...
		dataY = np.asarray(data_dict["actions"]) # now the actions are numpy array of ndata * (nact, which is max_var * 2)
		(self.num_data, self.max_clause, self.max_var, _) = dataX.shape
		
		# divide data into training and testing (May want to optimize on training size)
		ratio = 1.0 # Comments by Fei: too few training data for graph_coloring small data set, so just use everything as training.
		if nbatch > 0: # nbatch is given at construction time, smartly adjust num_train to be a multiple of nbatch
			self.num_train = int(self.num_data * ratio) // nbatch * nbatch 
		if nbatch == -1: # no nbatch information given
			self.num_train = int(self.num_data * ratio)
	...
	def next_batch(self, size, permute = False):
		if self.lastUsed + size > self.num_train:
			self.lastUsed = 0
			if permute:
				print(self.train["trainX"][100, 4, :, 0]) # Comments by Fei: just to prove that permutation happend. Can remove.
...
def super_train(filename, num_steps, nbatch, num_report, layer_norm, num_procs, num_model, permute = False):
	...
	with tf.name_scope("loss"):
		cross_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels = tf.nn.softmax(y_), logits = q_values))
		# L2 regularization
