...
def deepq_act_model(observations_ph, num_actions, layer_norm):
	...
	with tf.variable_scope("deepq", reuse=None):
		...
		neg = tf.reduce_min(observations_ph, axis = 1) # get -1 if the negative variables exists in any clauses, otherwise 0
		ind = tf.concat([pos, neg], axis = 2) # get (1, -1) if this var is present, (1, 0) if only as positive, (0, -1) if only as negative
		ind_flat = tf.reshape(ind, [-1, num_actions]) # this is nbatch * nact, with 0 values labeling non_valid actions, 1 or -1 for other
