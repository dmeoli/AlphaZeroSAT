...
def model(X, nact, layer_norm=False):
    """
    ...
    # h = layers.layer_norm(x, scale=True, center=True)
    if layer_norm:
        h = tf.keras.layers.LayerNormalization(scale=True, center=True)(h)
...
def model2(X, nact, layer_norm=False):
    """
...
def model3(X, nact, layer_norm=False):
    """
